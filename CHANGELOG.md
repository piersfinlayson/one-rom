...
- TODO (#277): 40-pin 16-bit ROMs are not yet supported by knock detection (which is inherently an 8-bit-ROM mechanism, matching against A0-A7 of single-byte accesses); the address-monitor tester skips 16-bit sets.  When that restriction is lifted, add a CHANGELOG entry and extend the tester and `ci/test-emu.sh` monitor pass to cover 16-bit ROMs.
