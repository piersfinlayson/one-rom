...
- RBCP (the host-control plugin) works again — the address monitor it builds on was completely broken by the v0.7.0 PIO rewrite — and now also supports 32-pin ROMs.
...
- Add image transforms — byte-level rearrangements applied to a ROM image before it is written into the firmware.  Two are available: `swap_bytes` (reverses the byte order within each 16-bit word, for 16-bit parts whose image was produced with the opposite byte order) and `deinterleave` (extracts one lane from an interleaved image, for a wide ROM set distributed as a single file — a 32-bit image split into 8-bit lanes, or into 16-bit halves).  A chip may now carry a `"transform"` array in a config file, and the CLI exposes the same via `--slot transform=...` using a `+`-separated text form (`transform=deinterleave:1/2/2+swap_bytes`).  The list is ordered and the order is significant.  `deinterleave` reads as "the image contains `stride` interleaved lanes of `bytes` bytes each; keep lane `offset`", with the parameters positional in that order and `bytes` defaulting to 1.  They are positional rather than named "high"/"low" because which half of a word an offset yields depends on the byte order of the source image, which is `swap_bytes`' job.  Names have aliases, accepted identically by the CLI and by a config file: `swap-bytes`/`swapbytes` for `swap_bytes`, `de_interleave`/`de-interleave`/`deint` for `deinterleave`, `trans=` for the `transform=` slot key, and `unit` for the `bytes` parameter.  Transforms run after any `location` window and after an Intel HEX image has been decoded, but before `size_handling` reconciles the image against the chip size; an odd-length image ahead of `swap_bytes` is resolved by `size_handling` `pad`/`truncate` or reported.  The transforms applied are recorded in the firmware metadata alongside the image's filename (as `kick.bin|transform=swap_bytes`), so a built image carries a record of how its ROM data was derived.  The implementation lives in `onerom-gen`, so any consumer that supplies the ROM through a config (CLI, Studio, Web) gets it.  Host-tooling change only; no firmware update required.
- Add `onerom image deinterleave`, the standalone counterpart to `onerom image swap-bytes`, for rewriting a file rather than transforming it during a build (`--offset`, `--stride`, `--bytes`).  Both subcommands now call the same `onerom-gen` implementation as the `transform=` slot key, so the standalone and in-build paths cannot drift apart.  Neither standalone subcommand reads Intel HEX yet; use `onerom image convert` first.
- `onerom-gen`'s public config model — `Config`, `ChipConfig`, `ChipSetConfig`, `Location`, `License`, `FileSpec`, `FileData` — and its `Error` enum are now `#[non_exhaustive]`, so future field and variant additions are backwards-compatible for downstream crates.  This is itself a breaking change (external code must construct these through the new `Config::new`, `ChipConfig::new`, `ChipSetConfig::new`, `Location::new` and `FileData::new` constructors rather than struct literals, and a `match` on `Error` must carry a wildcard arm), hence the `onerom-gen` minor version bump.  The workspace now enables `clippy::wildcard_enum_match_arm`, which stays quiet while every known variant is listed explicitly and fires as soon as a new one appears — so a new `Error` variant is caught by CI rather than silently absorbed by a wildcard.
- Fix `onerom-lab` applying the 8-bit read timing to the 27C200.  The longer read delay and tristate settle time belong to every 16-bit-capable part — it is A-1 taking part in address decoding when such a chip is read in byte mode that costs the extra cycles — but the test was written against `ChipType::Chip27C400` specifically, so the electrically identical 27C200 got 8 cycles and a 100-cycle settle instead of 12 and 200.  Now keyed on `supports_bit_mode(16)`.  Affects hardware testing only; no firmware or tooling change.
- Fix `size_handling: pad` filling the tail of an **Intel HEX** image with `0xAA` instead of `0xFF`.  The ihex support added earlier in this cycle documents the tail as reading `0xFF` — matching both the gaps its decoder already fills and an unprogrammed ROM cell — but the padding path hard-coded the raw-binary blank byte, so a padded ihex image ended up with two different fill values in it.  Raw binary images are unaffected (`0xAA` is what they pass in).  Found by review of the transform work, which made the disagreement visible inside a single image; there were no end-to-end tests of the ihex build path, and there now are.
...
- Expose the per-ROM ROM type string to plugins over the ORA plugin API.  The previously-reserved `ORA_ID_GET_FLASH_SLOT_EXT_INFO` is now implemented (`@since` firmware 0.7.1) as a per-ROM getter: for a given flash slot and ROM index it returns the ROM type string exactly as the user entered it (e.g. `27LC512`, not the canonical `27512`), plus the filename, chip size and RBCP type.  Previously the ROM type was only reachable as its numeric RBCP code (via `ORA_ID_GET_FLASH_SLOT_INFO`), so the exact spelling now preserved in the metadata could not be read back by any plugin.  Additive plugin-API change; the `api.h` version is unchanged.
  - This required a firmware update.
- Fix the `sel_jumper_pull` bitfield in the fire-28-c, fire-28-d and fire-32-b hardware configs, breaking image select jumpers B and D.  All had the high bit on the wrong SEL pin (`[0, 1, 0, 0]`): it sat on a grounded pin (SEL_D) instead of the SWDIO image-select pin, which is jumpered to the RUN line and so is pulled high through a resistor.  As a result the SWDIO select jumper was read inverted on those boards.  Corrected to `[0, 0, 0, 1]`, matching the identically-wired fire-40-a/b and fire-24-f boards.  This changes the generated firmware for those three boards.
- One ROM Lens works again, and has been ported from the old C-to-WebAssembly shim to a Rust crate (`onerom-lens`, in `rust/lens`) built on `onerom-fw-emulator` — it was untested and broken in v0.7.0.  It compiles to WebAssembly (via Emscripten) and runs the real firmware PIO/DMA serving code, visualising address/data/control-line waveforms live in the browser.  It handles 8-bit ROMs and the 27C400 in both byte (`/BYTE` low) and 16-bit word modes, shows the PIO program disassembly, and correctly displays hardware-inverted GPIOs.  Build and serve it with `rust/lens/serve.sh [CONFIG] [BOARD]`; the WebAssembly build is now covered by CI (`ci/build-lens.sh`).  The old `firmware/lens/` C shim and `firmware/lens.mk` have been removed.  No firmware update required (the serving code is reused at build time, the shipped firmware is unchanged).
...
- Improve One ROM Lens waveform readability, reworking the suggestions from PR #239 cleanly onto the Rust/wasm port rather than taking them as-is.  Colours: distinct green HIGH / cyan LOW levels (previously both drawn the same), neutral-grey High-Z and transition edges, and a black background; the dead `COLORS.grid`/`smeared*` loads the ported code never used are removed.  Timing readouts: the address/data hex rows now annotate each held value with its duration — in cycles (measured from the run, so unaffected by any left-edge clipping) and in nanoseconds derived from the firmware's real SYSCLK — and cycle-boundary graduation ticks appear once zoomed in far enough to distinguish individual cycles.  Cursor: compact, colour-coded per-bit values inline at each trace, plus a stacked summary (cycle, real time, and the address/data bus with ASCII) that follows the pointer, replacing the previous stack of oversized boxes.  The real-time figures read the clock rather than assuming one: `onerom-fw-emulator` gains `Emulator::sysclk_mhz()` (via the existing `ORA_ID_GET_SYSCLK_MHZ` plugin-API path), surfaced to the browser through a new `onerom_get_sysclk_mhz` Lens export and shown in the status panel.  Backwards-compatible; `onerom-fw-emulator` and `onerom-lens` remain unpublished.  No firmware update required.
- Fix the One ROM Lens signal-label hover tooltip showing `GPIO [object Object]` instead of the GPIO number — it stringified the whole `{pin, inverted}` pin-map entry rather than its `.pin`.  Lens-only; no firmware update required.
...
- Recognise `9316A` as an alias of the `2316` mask ROM (alongside the existing `9316`), so an Apple II/II+ ROM stamped `9316A` resolves without editing the config.  Data-only addition to `onerom-config`'s chip table; `docs/CHIP-TYPES.md` and `docs/COMPATIBILITY.md` are regenerated.  Host-tooling change only; no firmware update required.
- Fix a v0.7.0 regression that rejected valid multi-ROM sets depending on the order of their chips.  `onerom-gen`'s multi-set consistency check used the first *secondary* chip as its reference and compared the remaining chips against it, so a set mixing chip types with different numbers of control lines validated only when the chip with more lines came last.  For example a C64 set of Kernal (2364), Character (2332, whose CS2 is tied high in the machine and so set to `ignore`) and Basic (2364) validated with the character ROM last but was rejected — `Multi set secondary chips must all ignore the same control lines` — with it in the middle.  The check now anchors on the primary chip (chip 0), which is the chip the serving layout is actually derived from, and validates each secondary independently, so it accepts every ordering the firmware can serve and no longer depends on chip order.  It also now rejects, with clearer messages, multi sets the firmware could not serve: a secondary with more than one active control line, secondaries that select on different lines, or a secondary missing one of the primary's control lines.  Host-tooling change only; no firmware update required.

- Add a physical jumper-header descriptor to the board metadata, so host tools can draw an accurate image-select jumper diagram per board revision instead of assuming one fixed layout.  Each board's hardware JSON (`rust/config/json/*.json`) may now carry an optional `jumper_header` describing the 2xN header column by column (1-based, left-to-right), each column's top/bottom pads — and an optional third-row pad — tagged with roles (`5v`, `gnd`, `run`, `bootsel`, `sel_a`..`sel_e`, `swclk`, `swdio`, `x1`, `x2`, `a<N>` for a high address line broken out on the header, or the `nc`/`np` states).  The third-row pad is an X pin on 24/28-pin boards and a high address line (e.g. A17/A18) on 32-pin boards.  `onerom-config` exposes it as `Board::jumper_header() -> Option<JumperHeader>` (with new public `hw::{JumperHeader, HeaderColumn, HeaderSlot, HeaderRole}` types); boards not yet characterised return `None` and a consumer falls back to generic text.  At build time the descriptor is cross-checked against the board's existing electrical data — the image-select bits must match the `sel` pins, the `swclk`/`swdio` roles must sit on the select pads whose GPIOs are `swclk_sel`/`swdio_sel`, and the `x1`/`x2` roles must match the board's X pins — so the physical and electrical data cannot drift apart.  `one-rom-wasm`'s `board_info()` gains a matching `jumper_header` field.  Additive host-tooling/metadata change (both `onerom-config` and the WASM binding); no firmware update required.  The Fire 24, 28 and 32-pin boards are characterised; the remaining boards (Fire 40, Ice) are additive JSON to follow.
- Add CLI ASCII views of a board's physical pin layouts, built on the `jumper_header` metadata above.  `onerom board header [<board>]` draws the pin (jumper / programming) header pad by pad — with the MCU GPIO behind each image-select and X pad and, on RP2350 (Fire) boards, whether that GPIO is 5V-tolerant (`5V`) or 3.3V-only (`!!3V3!!`, an ADC pin).  `onerom board socket [<board>] [--chip-type <chip>] [--gpio]` draws the ROM socket as a DIP pinout: without `--chip-type` each pin shows the GPIO(s) behind it, with `--chip-type <chip>` it shows that ROM's functions (address / data / chip-select / `BYTE` / power), and `--gpio` overlays both; a multiplexed pin (e.g. the 27C400's pin 29, `A0/D15`) shows both functions.  When the chip's pin count differs from the board's, the socket is drawn at the larger of the two with the smaller device bottom-justified, matching `docs/COMPATIBILITY.md`: a smaller ROM on a larger One ROM marks One ROM's hanging-out pins `overhang`, and a larger ROM on a smaller One ROM marks the socket pins One ROM cannot reach `(empty)` and shows which One ROM `X1`/`X2` pin each overhanging address line must be fly-leaded to (e.g. `A12 → X1`).  In both cross-size cases the `--gpio` overlay also annotates the pin One ROM's own VCC/GND lands on (e.g. `NC (VCC)`), and the note points at the power-reroute requirement, since One ROM's power pins may not line up with the ROM's.  This reuses `onerom-gen`'s socket geometry (`onerom_gen::socket_pin_offset`, newly re-exported, and `compat::check_chip_on_board` to gate the fly-lead types).  The board is inferred from a connected One ROM when omitted, and the same views are available device-side as `onerom inspect header` / `onerom inspect socket`.  To support the tolerance annotation, `onerom-config`'s `mcu` module gains a public `PinTolerance` enum plus `RpVariant::adc_gpios()`/`gpio_tolerance()` and `Board::gpio_tolerance()`, modelling the RP2350 ADC pins (GPIO26-29 on the RP2350A, GPIO40-47 on the RP2350B) as the only pads that are 3.3V-only rather than 5V-tolerant; STM32F4 boards are not characterised and return `None`.  Additive `onerom-config` and CLI change; no firmware update required.  `docs/VOLTAGE-LEVELS.md` gains the ADC-pin caveat.

- Fix a v0.7.0 regression that left the address monitor — and therefore RBCP (the host-control plugin) — completely non-functional.  The v0.7.0 PIO rewrite moved the two address-monitor state machines out of a dedicated PIO block into the ROM-serving address and CS/data blocks (keeping the third PIO block free for user plugins), but the capture DMA, the CS→address-read IRQ handshake, and the state-machine enable were all left targeting the old, now-unused block: the DMA drained an empty FIFO, the "CS active" IRQ was set and waited in different PIO instances, and the wrong block's SMs were enabled — so no address capture ever reached the ring buffer and no knock was ever detected.  All three now derive their block, and the cross-instance PREV/NEXT IRQ routing, from the runtime serving-block assignment, so the monitor follows the state machines wherever they are placed.  The address monitor is also now extended to 32-pin ROMs, whose address pins sit in the upper GPIO bank (base 16) while CS/data stay in the lower bank (base 0): the CS monitor and address-read monitor live in separate blocks and each uses the GPIO base appropriate to the pins it reads, lifting the previous same-base restriction.  This required a firmware update.  A new emulator test (`onerom-fw-tester`'s `address-monitor-tester`, wired into `ci/test-emu.sh`) drives the address-monitor plugin API against the PIO/DMA emulator — capturing bus accesses and detecting a knock — across 24/28/32-pin ROMs and every board revision, so this path is covered from now on; supporting it added an epio capture-DMA ring channel (`epio_dma_setup_capture_pio_ring`) and firmware/emulator seams for the monitor DMA and ring write position plus a generic test-yield hook.
- Extend the address monitor to 40-pin 16-bit ROMs (#277).  Knock detection was previously assumed to be an 8-bit-ROM mechanism and the address-monitor tester skipped 16-bit sets; in fact the monitor observes the ROM's *word* address lines, so once command decode works in that observed space (see the observed-address entry below) 16-bit ROMs work like any other.  The tester's 16-bit skip is removed and each 16-bit set is now exercised in both `/BYTE` modes — word (`/BYTE` high) and byte (`/BYTE` low) — with a check that driving A-1 (the byte-within-word select) either way produces an identical captured address, i.e. that A-1 cannot leak into command decode.  Covers 27C400 and 27C200 on fire-40-a/b, with and without `force_16_bit`.
- Fix the emulator test harness silently testing the wrong ROM image whenever a run's first case was not image 0.  `stub_set_sel_image()` resolved the board's image-select GPIOs through the firmware's `MAX_GPIOS`, which is `max_gpios[RUNTIME->rp235x]` and only valid once `firmware_main()` has applied the RP235x variant; the first call in a process runs before any boot, so it judged an RP2350B board's sel pins (GPIO 38-41) out of range, drove no pins, and wrapped the requested index to image 0.  Any tester whose first executed case was not set 0 — the address-monitor tester skipping leading sets, for instance — then booted a different ROM and reported the outcome under the skipped-past set's label.  The stub now uses the variant `stub_set_rp_variant()` gave it rather than firmware runtime state, and all three emulator testers check after boot that the firmware selected the image the case asked for.  A sel value the board's pins cannot express still wraps, as on hardware: the PIO tester already accounted for that (oracle substitution, one-beyond test) and the other two now skip such a set with a note rather than exercise the wrong ROM.  Test-harness change only; no firmware or plugin update required.
- Support qualifier-based chip-select (`ALG_CS_2`) in the address monitor, so RBCP and the host-control plugin now work with the 23QL384 (#278) — the one ROM type that folds address lines into its chip-select decision and is therefore deselected over part of its address space.  The monitor's CS state machine gains an `ALG_CS_2` arm mirroring the serving one: it waits for the enable line, then compares the qualifier pins against the deselect pattern, and raises the capture IRQ only where the chip is genuinely selected.  Like the `AlgCs0`/`AlgCs1` monitor arms it debounces — four consecutive active samples — but only on the enable line, since the qualifiers are address lines that settle ahead of it and are read once the enable has held low.  A host walking the address from a deselected range into a served one without releasing the enable is a fresh access and produces a fresh capture.  The captured-entry checks follow: `demangle_observed_addr` now verifies the enable is asserted and the qualifier field does not read the deselect pattern (rejecting an entry from a range the chip never answered), and `init_knock` masks the enable line while leaving the qualifier address bits alone.  Command signalling must stay inside a served range — for the 23QL384, below the top quarter of its address space — which the host-control README now documents.  `map_addr_to_phys` still rejects `ALG_CS_2` for Multi sets: a single enable plus address qualifiers cannot express a Multi set's per-chip select, and the generator rejects every Multi 23QL384 combination on every board that supports the chip.  This required a firmware update; no plugin update is needed, as an existing host-control binary picks the support up from the firmware.  The address-monitor tester's skip list is now empty, and each `deselect_when_address_all_high` chip additionally has to prove that an access inside its deselected range produces no ring entry while one just below it captures normally.
- Correct the host-control plugin's documented RBCP limitations (#278): the **23QL384** is the only ROM type that resolves to the qualifier-based chip-select (`ALG_CS_2`) the address monitor does not yet support, and it does so on every board and in every CS configuration.  The **23QL512** was also listed — supposedly on board revisions whose upper address pins double as bank-select pins, with `cs1` active-low — but it carries no `deselect_when_address_all_high` and resolves to a plain chip-select everywhere; RBCP works with it.  The claim traced back to a test failure produced by the image-select bug above.  The address-monitor tester now exercises 23QL512 instead of skipping it, verified single and banked, `cs1` active-low and active-high, on fire-28-a/b/c/d.  Documentation and test-coverage change only; no firmware update required.
- Fix `onerom-gen`'s v2 (v0.7.0+) builder not checking that the composed ROM data fits the target board's flash.  The v1 builder rejected an over-large ROM set with `Error::BufferTooSmall`, but the v2 path computed the ROM data size and allocated its buffer without ever comparing it against the space left after the firmware and metadata region — so an oversized v2 config was caught only by the downstream `onerom-fw::validate_sizes`, which the CLI and the `onerom-fw` tool call but Studio and the web programmer (via `one-rom-wasm`) do not.  The check now lives in `build_v2` itself, mirroring the v1 guard, so every consumer of the single `onerom-gen` `build()` implementation gets it.  Host-tooling change only; no firmware update required.
- Add an observed (bus) address space to the plugin API and route RBCP command decode through it — the device-side groundwork for 16-bit (word-organised) and 40-pin ROMs.  On the 40-pin variant the device does not observe the ROM's least-significant address line (the byte-within-word select on a ×16 part such as the 27C400/27C200, or A0 on an 8-bit 40-pin part), so command signalling occupies a narrower address space than the byte-addressed ROM image.  Two additive plugin-API getters expose this — `ORA_ID_DEMANGLE_OBSERVED_ADDR` and `ORA_ID_GET_UNOBSERVED_ADDR_BITS` (both `@since` firmware 0.7.1).  host-control now demangles captured commands in observed space — previously it used byte-address space, where the omitted bit corrupted the command decode on a 16-bit ROM — and bounds the command page to the observed span.  The RBCP spec (`rom-bus-control-protocol`, now v0.1.1) gains a matching "Address Line Presentation" clarification.  The command decode this changes is covered for 16-bit ROMs by the emulator tests (see the address-monitor entry above) — the knock and the GROUP/CMD bytes following it, in both `/BYTE` modes.  The host-control plugin's wider command-response handling (ENTER_CMD_RESP, the back-channel, command dispatch) has no automated coverage for any ROM width, as before.  This required a firmware update and a host-control plugin update.

- Make the flash a chip type costs queryable from the CLI, rather than only readable from `docs/COMPATIBILITY.md` or discoverable after building an image.  `onerom chips --board <board>` now lists each chip the board can emulate with its ROM size and its **image size** — the flash One ROM uses to emulate it — grouped by how the chip fits the socket (native / overhang / fly-lead), and `--chip-type <chip>` answers for a single type.  The figure matters because it is frequently larger than the chip: a 2364 costs 8KB on a 24-pin board but 256KB overhanging a 28-pin one, so what fits in flash is not obvious from the ROM sizes.  `onerom board socket --chip-type <chip>` (and `inspect socket`) reports the same figure below the pinout.  The listing is now the full set the board can emulate, including the overhang and fly-lead combinations the previous name-only list omitted, and a recognised chip type the board cannot serve (the SRAM types, which no v0.7.x firmware serves) is named separately rather than listed as supported; Ice (STM32) boards, which the v2 compatibility derivation cannot size, keep the plain name list.  To support this, `onerom-gen`'s `compat` module gains `supported_chips`, `format_size`, `pin_offset_order` and `CompatResult::fit_description`, which the `compat` binary that generates `docs/COMPATIBILITY.md` now shares with the CLI, so the tool and the document cannot disagree (the generated document is unchanged).  Sizes are for a chip served alone in its slot; a banked or multi-ROM set draws further lines into the slot's address window and can be larger.  Host-tooling change only; no firmware update required.
- Give `onerom-gen` a single definition of the flash available for ROM images, `rom_data_space(mcu_variant)` — the flash less the firmware (48KB) and metadata (16KB) regions, so 1984KB on the RP2350.  Both builder paths open-coded this arithmetic, with the V1 path bounding it by `MAX_METADATA_LEN` and the V2 path by `METADATA_SIZE`; the two constants are equal, which is now asserted at compile time so the shared helper cannot silently become wrong if they diverge.  No behaviour change.
- Reword the socket fit reported for a chip with more pins than the board whose extra pins carry no address lines — the 32-pin 28C512 on a 28-pin board, where One ROM sits bottom-justified in the larger socket and no address line needs wiring.  Both `docs/COMPATIBILITY.md` and `onerom chips` described it as `no fly-leads required`, directly beneath a `(with fly-leads)` section heading, which reads as a contradiction; it now reads `larger socket (no fly-leads)`, stating where One ROM sits as well as what it does not need.  Note this remains a cross-size fit: the socket's VCC is among the pins One ROM cannot reach, so power must still be rerouted.  `docs/COMPATIBILITY.md` is regenerated (four cells, on the four 28-pin boards).  Wording change only.
- Document what the per-board **Fit** column can say.  `docs/COMPATIBILITY.md`'s per-board tables and `onerom chips` both carry a Fit column, but only the compatibility matrix's cell markers (`64KB`, `64KB*`, `64KB†`) were ever legended; the four Fit values were not.  Both `docs/COMPATIBILITY.md` (via its generator) and `docs/CLI-MANUAL.md` now carry a legend for `native`, `overhang`, `larger socket (no fly-leads)` and `fly-lead to X1`/`X1 and X2`, and both spell out that every non-native fit may need power rerouted to One ROM's own VCC/5V pin — `larger socket (no fly-leads)` means no *signal* wiring, not that the chip drops in.  The CLI manual's `chips` example output is also replaced with a verbatim run, which adds the footer, the "Recognised but not servable on this board" line and the plugin-types block it previously omitted.  Documentation only.
- Give plugins a GPIO drive and query API.  Two additive plugin-API calls (`@since` firmware 0.7.1), `ORA_ID_GPIO_SET` and `ORA_ID_GPIO_QUERY`, let a plugin drive a GPIO high or low, release it to high impedance, and read back what One ROM is using it for along with its level and direction.  The immediate use is hard-resetting the host system One ROM is installed in — a wire from a header pad to the machine's reset line — but the primitive is general, and the CLI's `control pin` and `control reset` will be built on it.  `ora_gpio_set` refuses a GPIO One ROM is itself using unless `ORA_GPIO_FLAG_FORCE` is passed; that check has to live in the firmware, because `setup_serving_gpios` configures address and chip-select pins as SIO inputs with the output driver disabled, making them indistinguishable from unused pins by register inspection, and because X pins are folded into the address span on Multi and Banked slots and so are only sometimes free.  What the query reports is the consequence of taking a pin over rather than the role it plays — the data pins are driven by PIO, so forcing one moves its function select away from PIO and serving stays broken until reboot, while everything else serving uses is an input that PIO keeps reading regardless, so forcing one is reversible.  Naming a pin's role is left to the host, which already has the board pin map and chip metadata.  Only the function select, the output enable and the pad's output-disable bit are altered, so pulls, drive strength, slew and the input polarity inversion serving applies to select lines all survive a force-then-release.  Additive plugin-API change; the `api.h` version is unchanged.  Also fixes a latent fault in `retrieve_gpio_init`, which added the chip-select algorithm's GPIO base to the `/BYTE` pin parameter unconditionally: with no `/BYTE` pin that parameter is `GPIO_NONE`, so a base of 16 wrapped it round to GPIO 15, which serving would then have reconfigured.  It is unreachable on today's boards.  The plugin API tester gains coverage of the classification across both RP2350 variants and every chip-select and data algorithm, checked against what the serving setup actually configured rather than a second reading of the same configuration; Multi and Banked slots, previously skipped by that tester outright, now run it.
  - This required a firmware update.
- Give the USB system plugin the device side of GPIO control, over three One ROM picobootx commands: `ONEROM_CMD_GET_CAPS`, `ONEROM_CMD_GPIO_SET` and `ONEROM_CMD_GPIO_QUERY`.  A host can now drive a One ROM GPIO high, low or high impedance, optionally for a bounded period, and read back what One ROM is using each of its GPIOs for; the CLI surface that uses this follows.  The bounded hold is timed on the device rather than by the host sleeping between two commands, because the host may not survive the hold — if the CLI is interrupted, the terminal closes or the cable is pulled mid-pulse, a host-timed pulse would leave the target latched, which for the motivating case (a wire from a header pad to the host system's reset line) means held in reset indefinitely.  Up to eight GPIOs can be held at once, for at most 60 seconds each — a longer assertion is better expressed as an indefinite latch and an explicit release, since a hold only lasts as long as the plugin runs — and a second `ONEROM_CMD_GPIO_SET` on a GPIO replaces that GPIO's pending release rather than accumulating one.  `ONEROM_CMD_GPIO_SET` is validated and applied in the picoboot dispatch handler rather than deferred to the plugin task loop the way `ONEROM_CMD_SET_LED` is: dispatch already runs on the plugin's own core in the task loop's context, and applying it there is the only way the firmware's refusal of a GPIO One ROM is itself using reaches the host as a command status instead of being swallowed.  What the plugin can offer is decided once at init from which plugin-API functions the running firmware provides, so on firmware that predates the GPIO API the capability bits are clear and the host never sends the commands — the plugin's `min_fw_version` therefore stays at 0.7.0 and everything else it does keeps working, the same runtime-detection approach the serial override already uses.  This also required relaxing the custom-command dispatcher's length check, which rejected any command carrying a data phase at all before it looked at the command ID; it now validates the length per command, and a malformed command is still rejected, just not on the grounds of having a data phase.  The number of GPIOs the device has — 30 on an RP2350A, 48 on an RP2350B — comes from a new metadata plugin key, `ORA_METADATA_KEY_RP_VARIANT`, exposing the RP235x variant the firmware detected at boot; that is the same field the firmware's own `MAX_GPIOS` is indexed by, so the count the device advertises and the range its plugin-API calls accept cannot drift apart.  Depends on picobootx's new optional `fill` callback, without which a custom command could not return data to the host at all.
  - This required a firmware update and a USB plugin update.
- Give the CLI the user-facing side of GPIO control: `onerom control pin` drives a One ROM GPIO high, low or high impedance, `onerom control reset` pulses one low to reset the host system One ROM is installed in, and `onerom inspect gpio` shows what every GPIO is and what One ROM is doing with it.  This completes the feature the two entries above build up to; the primary use is a wire from a header pad (typically X1/X2, or an image-select pad whose jumper has been removed) to the machine's reset line, so a scripted workflow can program a new image and then reset the machine into it.  `control pin --hold <MS>` holds the state for a bounded period and then applies `--then`, defaulting to high impedance; without `--hold` the state latches until something changes it.  The hold is timed by the device, not by the CLI sleeping between two commands, so an interrupted CLI cannot leave a pin latched.  `control reset` is that with the state fixed low, `--then` fixed to high impedance and `--hold` defaulting to 100ms; it never drives the line high, because a reset net has its own pull-up and may have other drivers on it, and it rejects a `--hold` of 0 rather than honouring it as an indefinite latch, since a reset pulse with no end is not a reset.  A GPIO One ROM is itself using is refused, naming what it is doing, unless `--force` is given, and forcing prints what it costs — a pin serving *reads* (address, chip-select, `/BYTE`) is put back with `--state z`, while a pin serving *drives* (a data pin) leaves serving broken until the device is rebooted.  A GPIO that is not 5V-tolerant (an RP2350 ADC pin, from static board metadata, not a measurement) warns and asks, which `--yes` or `--force` answers; nothing else about the pad is checked, because what is wired to it, whether a jumper is fitted and what voltage the far end sits at are the user's to know and not something the device can determine.  `--pin` takes an MCU GPIO written `gpio<N>` only: a bare number is rejected rather than guessed at, since `23` is a plausible GPIO, image-select pad, X pad and ROM socket pin all at once and driving the wrong one is not recoverable, and the header pad names (`sel_a`, `x1`, ...) are recognised and reported as not yet supported rather than as meaningless.  `inspect gpio` names each GPIO's role itself, from the board pin map and the chip type being served, because the device deliberately reports only the consequence of taking a pin over (free / read by serving / driven by serving / system) and never the role it plays; the table's `Function` column reuses the same board lookups `onerom board header` and `onerom board socket --gpio` draw with, so the diagrams and the table cannot drift apart, and a board revision, ROM type or header layout this build cannot resolve costs those names rather than the listing.  The GPIO count listed and accepted is the device's own, 30 on an RP2350A and 48 on an RP2350B, read from it rather than assumed.  All three commands require a *running* One ROM with the USB system plugin, and say so precisely: a stopped One ROM sits in the RP2350 bootloader, where One ROM's own command handler does not exist at all.  Against firmware or a USB plugin that predates GPIO control the CLI reports which of the two is too old instead of surfacing a USB stall.  Host-tooling change only; it needs firmware v0.7.1 or later and the v0.7.1 or later USB system plugin on the device.  `docs/CLI-MANUAL.md` documents all three, plus a shared reference for what `--pin` accepts.
- **Breaking change: `onerom boards` is now `onerom board list`.**  There is deliberately no alias, so a script calling `onerom boards` must be updated — the CLI suggests `board` rather than simply failing.  A plural noun taking a singular argument read wrongly (`onerom board header fire-24-f` describes one board), and with three subcommands beneath it the listing deserved a name of its own rather than being what you got by omitting one.  The `board header` and `board socket` views added earlier in this release are documented above under their shipping names and never existed as `boards` subcommands in a release.  The rest of this entry is presentation, corrected after the GPIO feature was exercised on real hardware.  `onerom inspect gpio` collapses its `Pad` and `Function` columns into a single `Function` column listing everything the GPIO is, in a fixed order — the ROM socket signal under the image being served, then the board peripheral, then the header pad — deduplicated by name.  The old split was about where a name came from rather than about anything a reader needs: on a 32-pin board the `a17` header pad and the socket's `A17` line are one net, so the two columns said the same thing, and everywhere else exactly one of them was populated.  An X or image-select pad is a function of the pin like any other, and header pads stay in the table because "which GPIO is X1" is the main thing it is read to answer before wiring a reset line.  That collapse also fixes a real omission: the system-function lookup returned on its first match, so on a fire-24-f — where the Status LED and the RGB LED are both GPIO 29 — the table named only the Status LED, understating what driving the pin most likely to be driven by accident would disturb; both are now listed, as they are in `control pin`'s refusal message.  In the other direction, the column no longer claims a GPIO is `SWCLK` or `SWDIO`: those are dedicated RP2350 pins with no GPIO of their own, and the `SEL_C`/`SEL_D` pads merely share their nets, which is a fact about the pad rather than about the GPIO — `onerom inspect header` remains the pad-indexed view and still shows every role each pad carries.  `inspect gpio` now lists only the GPIOs connected to something and says how many it omitted, with `--all` for the full listing: thirteen of a fire-28-c's forty-eight GPIOs are connected to nothing at all and buried the rows worth reading.  The filter asks what a GPIO *is*, not what the device reports using it for, because the `X1`/`X2` and image-select pads report `free` and are exactly what the table exists to show; with no recognised board nothing is hidden, since nothing can be ruled out.  Finally, the explanatory legend beneath the table moves behind the existing global `--verbose`: it is worth reading once and noise on every run after that, and nothing is lost by default, since the cost of taking a serving pin over is stated at the point of action by `control pin` itself.  Host-tooling change only; no firmware or plugin update required.  `docs/CLI-MANUAL.md` and `docs/VOLTAGE-LEVELS.md` are updated.
- `onerom control pin`, `onerom control reset` and `onerom inspect gpio` now accept header pad names on `--pin`, alongside `gpio<N>`: `sel_a` to `sel_e` for the image-select pads (`sel-a` and `sela` are accepted too) and `x1`/`x2` for the X pads.  These are the pads a user can physically put a wire on, and the reset use case is exactly "the pad my wire is soldered to", so making the user look up the GPIO behind a pad they can name was busywork.  Because clap parses `--pin` before any device is opened, and a pad name means nothing without a board, parsing and resolution are separate steps: the parser is board-free and yields either an MCU GPIO or a named pad, and the command handler resolves the pad once the board is known — so the type carrying a GPIO number is only ever produced where a board was available, and the question cannot be asked of an unresolved pad.  Resolution reads the board's electrical pin assignments rather than its jumper-header layout, because those exist for every board including the non-USB Ice boards whose header is deliberately uncharacterised, so a pad name resolves on any board that has the pad.  A board with no such pad — `sel_e` on a four-select board, `x1` where the board has no X pads — is refused, naming the pads it does have; a pad named when no board could be determined is refused pointing at the new `--board` option, which those three commands gained for exactly this case and which overrides what the device reports.  A bare number is still rejected: accepting pad names sharpens rather than removes the ambiguity that made `--pin 23` an error.  The broken-out address pads (`a<N>`) remain recognised but are now refused as a settled decision rather than as "not yet supported": `--pin` addresses MCU GPIOs and the pads a wire can reach, an address line is a ROM signal rather than one of those, and accepting `a17` would invite `a11` or `d3`, which have no pad at all.  Separately, `control pin --state` and `--then` now accept `1` and `0` as aliases for `high` and `low`.  Host-tooling change only; no firmware or plugin update required.  `docs/CLI-MANUAL.md` is updated, including its shared `--pin` reference.
- Split the Ice (STM32) boards out of the CLI's board listing, and refuse them where the CLI cannot use them.  `onerom board list` — and `onerom scan --list-boards`, which now prints the same listing rather than its own one-liner — shows the Fire boards under the existing heading and the Ice boards under a second one that says what they are.  The CLI has never been able to act on an Ice board: every firmware path composes an image for the RP2350 and every device path speaks picoboot, which is the RP2350 bootloader, so there is no image to build and no bootloader to reach.  A single merged list implied otherwise, and `--board ice-24-d` then failed several layers down as a release the manifest does not have, which names a symptom rather than the cause.  `scan`, `program`, `firmware build`, `firmware download`, `firmware inspect --board`, `control pin`, `control reset` and `inspect gpio` now reject an Ice `--board` where the user named it, saying which boards they do take; on the three GPIO commands the board only labels pins, so an Ice board there would have silently named a connected Fire's GPIOs against the wrong hardware.  The commands that only *describe* hardware are unchanged and still accept Ice boards: `board header`, `board socket`, `chips` and `firmware releases`, none of which builds an image or opens a device.  Note that this is narrower than the v0.7.0 note that "the programming tools continue to support Ice boards and v0.6.xx" — that holds for the web programmer and Studio, but the CLI never implemented an STM32 path.  Host-tooling change only; no firmware or plugin update required.  `docs/CLI-MANUAL.md` is updated.
- Stop hard-wrapping prose in the CLI's console output.  A handful of messages broke a sentence across lines at a fixed width — the `onerom chips` image-size note, three of the `--pin` errors and the new Ice-board one — and the terminal then wrapped the result again at its own width, so the block came out half-wrapped.  Each sentence is now emitted as one line and the terminal decides where it breaks.  Messages that put a *separate* sentence on its own indented line are unchanged, as are the `--slot` and `--fw-version` errors that echo the offending argument on a following line: that is structure rather than wrapping.  `docs/CLI-MANUAL.md` is updated where it quotes the affected output.
- Make `onerom board socket` and `onerom inspect socket` report a board with no GPIO map rather than drawing around it.  The two views that label socket pins with GPIOs — no `--chip-type`, and `--chip-type` with `--gpio` — are drawn from the board's socket pin map, and a board without one produced a diagram with the GPIO column blank all the way down, which reads as "this pin has no GPIO" rather than "this board has no map".  They now say so and draw nothing.  The `--chip-type` function view is unaffected, being drawn from the chip's pinout and the board's ROM signal assignments instead.  Host-tooling change only; no firmware or plugin update required.  `docs/CLI-MANUAL.md` is updated.  `onerom board header` and `onerom inspect header` likewise now say `command unsupported` for a board with no pin-header descriptor, in place of `nothing to draw` and a "yet" that promised the descriptor was coming: what a command does today is what it can report, and whether a board gains one later is not an error message's to announce.  `docs/CLI-MANUAL.md` drops the matching "not yet characterised" wording in both places it used it.
